...
int FIELDMAP::magnetlengths(magnetvec &dipols, const char *filename) const
{
  unsigned int w=10;
  unsigned int i, j=0;
  ...
  file << "n_samp = " << n_samp << " sampling points per turn" << endl << endl;
  file <<setw(w)<< "Name" <<setw(w)<< "start/mm" <<setw(w) << "end/mm" <<setw(w)<< "length/mm" << endl;


  for (i=0; i<n_samp; i++) {

    if (name(i) == dipols[j].name) {
      tmp_start = pos(i);
      while (name(i) == dipols[j].name) i++;
      tmp_end = pos(i-1);
